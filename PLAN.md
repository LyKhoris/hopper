...
- [x] **Phase 19 — no redundant work or words:** the fcitx order script checks order + cloud separately and only applies what's missing (a cloud-only re-run leaves the profile untouched). Per-item skip noise is hidden by default via `log_skip` in `modules/_lib.sh` (preview already listed it) — `HOPPER_VERBOSE=1` restores it, summaries always print.
